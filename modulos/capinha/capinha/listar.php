...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		...
		<div class="col-sm-2 hidden-xs">
			<input type="text" name="id_modelo" id="id_modelo" value="<?= $id_modelo ?>" class="form-control required" required readonly>
		</div>
		<div class="col-sm-8">
			<div class="input-group">
				<span class="input-group-btn">
				...
				<input type="text" name="modelo" id="modelo" value="<?= $modelo ?>" class="form-control required" required readonly placeholder="Informe o modelo...">
			</div>
		</div>
	</div>
	...
	try {
		...
		$sql = "SELECT capinha.*,
			 ...
			 LEFT JOIN capinha_marca 
				 ...
				 WHERE capinha.status_registro = :status_registro ";
		...
		if($id_marca != "") {
			...
			$sql .= "AND capinha.id_marca = :id_marca ";
		...
		if($id_modelo != "") {
			...
			$sql .= "AND capinha.id_modelo = :id_modelo ";
			$vetor['id_modelo'] = $id_modelo;
			$link .= "&id_modelo=" . urlencode($id_modelo);
		...
		if($descricao != "") {
			...
			$sql .= "AND capinha.descricao LIKE :descricao ";
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th style="width: 90px;">
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=capinha.id&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						C&oacute;digo
					</a>
					<?php if($o == "capinha.id" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "capinha.id" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=capinha.descricao&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						T&iacute;tulo
					</a>
					<?php if($o == "capinha.descricao" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "capinha.descricao" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('capinha', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				<option value="capinha.id" <?php if($o == "capinha.id") echo "selected"; ?>>C&oacute;digo</option>
				<option value="capinha.descricao" <?php if($o == "capinha.descricao") echo "selected"; ?>>T&iacute;tulo</option>
				<option value="capinha_modelo.modelo" <?php if($o == "capinha_modelo.modelo") echo "selected"; ?>>Modelo</option>
		...
		<table class="tb-sx-sm">
			...
			<tr>
                <td class="conteudo-sx-sm">
                    ...
                    <span class="topico-sx-sm">Modelo</span><br>
					<?= $buscaArtigo['modelo'] ?>
