<?php
	if($_REQUEST['id_excluir']) {
		...
		excluir($_REQUEST['id_excluir'], "capinha_marca", array("capinha", "capinha_modelo"), array("id_marca", "id_marca"));
...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		<label for="marca" class="col-sm-2 control-label">Descri&ccedil;&atilde;o</label>
		<div class="col-sm-10">
			<input type="text" name="marca" id="marca" class="form-control" value="<?= $marca ?>" placeholder="Informe a marca...">
		</div>
	</div>
...
<?php if(empty($status_registro) || $status_registro == "A") { ?>
				...
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=marca&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('capinha_marca', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				...
				<option value="marca" <?php if($o == "marca") echo "selected"; ?>>Descri&ccedil;&atilde;o</option>
			</select>
