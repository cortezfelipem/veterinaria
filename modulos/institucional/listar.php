...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	$sql = "SELECT *
	          FROM institucional
	         WHERE institucional.status_registro = :status_registro ";
	...
	if($titulo != "") {
		...
		$sql .= "AND institucional.titulo LIKE :titulo ";
		$vetor['titulo'] = "%$titulo%";
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th style="width: 90px;">
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=institucional.id&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						C&oacute;digo
					</a>
					<?php if($o == "institucional.id" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "institucional.id" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=institucional.titulo&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						T&iacute;tulo
					</a>
					<?php if($o == "institucional.titulo" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "institucional.titulo" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('institucional', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				<option value="institucional.id" <?php if($o == "institucional.id") echo "selected"; ?>>C&oacute;digo</option>
				<option value="institucional.titulo" <?php if($o == "institucional.titulo") echo "selected"; ?>>t&iacute;tulo</option>
			</select>
