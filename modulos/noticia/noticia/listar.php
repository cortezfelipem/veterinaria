...
?>
	...
	<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<input type="text" name="titulo" id="titulo" class="form-control" value="<?= $titulo ?>" placeholder="Informe o T&itilde;tulo...">
			</div>
		</div>
							...
							<tr>
								<td class="text-center">
									<?php if($buscaArtigo['status_registro'] == "A") { ?>
										...
										<?php } ?>
									<?php } else { ?>
										<a href="#" class="text-info btn-lista" onclick="restaurar('noticia', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
									<?php } ?>
								</td>
								...
								<!-- <td>
									<?php
									// if ($buscaArtigo['tipo'] == 'F') echo "Foto destaque";
									// elseif ($buscaArtigo['tipo'] == 'U') echo "&Uacute;ltimas not&iacute;cias";
									// elseif ($buscaArtigo['tipo'] == 'E') echo "Especiais";
									?>
								</td> -->
