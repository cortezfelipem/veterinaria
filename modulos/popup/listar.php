...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" name="titulo" id="titulo" class="form-control" value="<?= $titulo ?>" placeholder="Informe o t&iacute;tulo...">
		</div>
	</div>
...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	$stPopup = Conexao::chamar()->prepare($sql);
	$stPopup->execute($vetor);
	$qryPopup = $stPopup->fetchAll(PDO::FETCH_ASSOC);
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			...
			<?php
			if(count($qryPopup)) {
			foreach($qryPopup as $buscaPopup) {
			?>
			<tr>
				<td class="text-center">
					<?php if($buscaPopup['status_registro'] == "A") { ?>
						<? if($excluir) { ?>
						<input type="checkbox" class="marcar" name="id_excluir[]" value="<?= $buscaPopup['id'] ?>">
						<a href="#" class="text-danger btn-lista" onclick="excluir('<?= $caminhoTela ?>&id_excluir=<?= $buscaPopup['id'] ?>');" data-toggle="tooltip" data-placement="right" title="Excluir Registro"><i class="fa fa-trash"></i></a>
						<?php } ?>
						<? if($cadastrar) { ?>
						<a href="<?= $caminhoTela ?>&id=<?= $buscaPopup['id'] ?>&s=cadastrar" class="btn-lista" data-toggle="tooltip" data-placement="right" title="Editar Registro"><i class="fa fa-pencil"></i></a>
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('audio', '<?= $buscaPopup['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
				<td class="text-right"><?= $buscaPopup['id'] ?></td>
				<td><?= $buscaPopup['titulo'] ?></td>
				<td><?= formata_data($buscaPopup['data_inicial']) ?></td>
				<td><?= formata_data($buscaPopup['data_limite']) ?></td>
			</tr>
			<?php }} ?>
		</table>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				...
				<option value="data_inicial" <?php if($o == "data_inicial") echo "selected"; ?>>Data Inicial</option>
		...
		<table class="tb-sx-sm">
			...
			<tr>
				<td class="conteudo-sx-sm">
					...
					<span class="topico-sx-sm">Data Inicial</span><br>
					<?= formata_data($buscaPopup['data_inicial']) ?>
			...
			<tr>
				<td class="conteudo-sx-sm">
					...
					<?= formata_data($buscaPopup['data_limite']) ?>
