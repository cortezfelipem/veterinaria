...
if($_REQUEST['id_excluir']) {
	...
	//excluir($_REQUEST['id_excluir'], "produto_atributo", "produto", "id_subcategoria");
...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		<label for="id_tipo_atributo" class="col-sm-2 control-label">Tipo_Atributo</label>
		<div class="col-sm-2 hidden-xs">
			<input type="text" name="id_tipo_atributo" id="id_tipo_atributo" value="<?= $id_tipo_atributo ?>" class="form-control" readonly >
		</div>
		<div class="col-sm-8">
			<div class="input-group">
				...
				<input type="text" name="tipo_atributo" id="tipo_atributo" value="<?= $tipo_atributo ?>" class="form-control" readonly placeholder="Informe o tipo de atributo...">
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" name="descricao" id="descricao" class="form-control" value="<?= $descricao ?>" placeholder="Informe a descri&ccedil;&atilde;o...">
		</div>
	</div>
...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	$sql = "SELECT  produto_atributo.*,
	     ...
	     LEFT JOIN produto_tipo_atributo ON produto_atributo.id_tipo_atributo = produto_tipo_atributo.id
	         WHERE produto_tipo_atributo.status_registro = :status_registro ";
	...
	if($descricao != "") {
		...
		$sql .= "AND produto_atributo.descricao LIKE :descricao ";
	...
	if(empty($o)) $o = "produto_atributo.descricao";
	if(empty($d)) $d = "ASC";
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th style="width: 90px;">
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=produto_atributo.id&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						C&oacute;digo
					</a>
					<?php if($o == "produto_atributo.id" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "produto_atributo.id" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=produto_atributo.descricao&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						Descri&ccedil;&atilde;o
					</a>
					<?php if($o == "produto_atributo.descricao" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "produto_atributo.descricao" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=tipo_atributo&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						Categoria
					</a>
					<?php if($o == "produto_atributo.id_tipo_atributo" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "produto_atributo.id_tipo_atributo" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
			...
			<tr>
				...
				<td><?= $buscaArtigo['descricao'] ?></td>
				<td><?= $buscaArtigo['tipo_atributo'] ?></td>
			</tr>
			<?php }} ?>
		</table>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				<option value="produto_subcategoria.id" <?php if($o == "produto_subcategoria.id") echo "selected"; ?>>C&oacute;digo</option>
				<option value="produto_subcategoria.subcategoria" <?php if($o == "produto_subcategoria.subcategoria") echo "selected"; ?>>Descri&ccedil;&atilde;o</option>
				<option value="produto_categoria.subcategoria" <?php if($o == "produto_categoria.subcategoria") echo "selected"; ?>>Categoria</option>
			</select>
