...
if($_REQUEST['id_excluir']) {
	...
	excluir($_REQUEST['id_excluir'], "produto_categoria", array("produto", "produto_subcategoria"), array("id_categoria", "id_categoria"));
...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		<label for="categoria" class="col-sm-2 control-label">Descri&ccedil;&atilde;o</label>
		<div class="col-sm-10">
			<input type="text" name="categoria" id="categoria" class="form-control" value="<?= $categoria ?>" placeholder="Informe a descri&ccedil;&atilde;o...">
		</div>
	</div>
...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	if($categoria != "") {
		...
		$vetor['categoria'] = "%$categoria%";
		$link .= "&categoria=" . urlencode($categoria);
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=categoria&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('produto_categoria', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				...
				<option value="categoria" <?php if($o == "categoria") echo "selected"; ?>>Descri&ccedil;&atilde;o</option>
			</select>
