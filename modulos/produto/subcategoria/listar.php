...
if($_REQUEST['id_excluir']) {
	...
	excluir($_REQUEST['id_excluir'], "produto_subcategoria", "produto", "id_subcategoria");
...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" name="subcategoria" id="subcategoria" class="form-control" value="<?= $subcategoria ?>" placeholder="Informe a descri&ccedil;&atilde;o...">
		</div>
	</div>
...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	$sql = "SELECT produto_subcategoria.*,
	     ...
	     LEFT JOIN produto_categoria ON produto_subcategoria.id_categoria = produto_categoria.id
	         WHERE produto_subcategoria.status_registro = :status_registro ";
	...
	if($id_categoria != "") {
		...
		$sql .= "AND produto_subcategoria.id_categoria = :id_categoria ";
	...
	if($subcategoria != "") {
		...
		$sql .= "AND produto_subcategoria.subcategoria LIKE :subcategoria ";
		$vetor['subcategoria'] = "%$subcategoria%";
		$link .= "&subcategoria=" . urlencode($subcategoria);
	...
	if(empty($o)) $o = "produto_subcategoria.subcategoria";
	if(empty($d)) $d = "ASC";
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th style="width: 90px;">
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=produto_subcategoria.id&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						C&oacute;digo
					</a>
					<?php if($o == "produto_subcategoria.id" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "produto_subcategoria.id" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
				...
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=produto_categoria.subcategoria&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						Categoria
					</a>
					<?php if($o == "produto_categoria.subcategoria" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "produto_categoria.subcategoria" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
			...
			<tr>
				...
				<td><?= $buscaArtigo['subcategoria'] ?></td>
