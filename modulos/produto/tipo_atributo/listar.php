...
if($_REQUEST['id_excluir']) {
	...
	excluir($_REQUEST['id_excluir'], "produto_tipo_atributo", array("produto_atributo"), array("id_tipo_atributo"));
...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" name="descricao" id="descricao" class="form-control" value="<?= $descaricao ?>" placeholder="Informe a descri&ccedil;&atilde;o...">
		</div>
	</div>
...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	$stm = Conexao::chamar()->prepare($sql);
	$stm->execute($vetor);
	$qryArtigo = $stm->fetchAll(PDO::FETCH_ASSOC);
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=descricao&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('produto_tipo_atributo', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				...
				<option value="descricao" <?php if($o == "descricao") echo "selected"; ?>>Descri&ccedil;&atilde;o</option>
			</select>
