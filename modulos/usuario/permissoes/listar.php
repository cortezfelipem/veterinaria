...
//	excluir($_REQUEST['id_excluir'],
//			"usuario",
//			array("controle_icone_usuario"),
//			array("id_usuario"));
//
//}
...
<form class="form-horizontal collapse" id="form-busca" action="<?= $caminhoTela ?>" method="POST">
	...
	<div class="form-group">
		<label for="usuario" class="col-sm-2 control-label">Nome</label>
		<div class="col-sm-10">
			<input type="text" name="usuario" id="usuario" class="form-control" value="<?= $usuario ?>" placeholder="Informe o nome...">
		</div>
	</div>
	...
	<div class="table-responsive hidden-sm hidden-xs">
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th>
					<a href="<?= $link ?>&p=<?= $pagina ?>&o=usuario&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						Nome
					</a>
					<?php if($o == "usuario" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "usuario" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('usuario', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				...
				<option value="usuario" <?php if($o == "usuario") echo "selected"; ?>>Usu&aacute;rio</option>
			</select>
