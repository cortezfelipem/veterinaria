...
<form id="form-lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
	...
	$sql = "SELECT *
	          FROM video
	         WHERE video.status_registro = :status_registro ";

	if($buscaAdministrador['master'] == "S" && $status_registro == "I") {

		$vetor["status_registro"] = "I";
		$video .= "&status_registro=I";

	} else $vetor["status_registro"] = "A";
	
	if($titulo != "") {

		$sql .= "AND video.titulo LIKE :titulo ";
		$vetor['titulo'] = "%$titulo%";
		$video .= "&titulo=" . urlencode($titulo);
	...
	<div class="table-responsive hidden-sm hidden-xs">
		...
		<table class="table table-striped table-hover table-bordered table-condensed">
			<tr class="active">
				...
				<th style="width: 90px;">
					<a href="<?= $video ?>&p=<?= $pagina ?>&o=video.id&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						C&oacute;digo
					</a>
					<?php if($o == "video.id" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "video.id" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				</th>
				<th>
					<a href="<?= $video ?>&p=<?= $pagina ?>&o=video.titulo&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>" data-toggle="tooltip" data-placement="top" title="Clique para ordenar">
						T&iacute;tulo
					</a>
					<?php if($o == "video.titulo" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
					<?php if($o == "video.titulo" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
			...
			<tr>
				<td class="text-center">
					<?php if($buscaArtigo['status_registro'] == "A") { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<a href="#" class="text-info btn-lista" onclick="restaurar('video', '<?= $buscaArtigo['id'] ?>', '<?= $caminhoTela ?>');" data-toggle="tooltip" data-placement="right" title="Restaurar Registro"><i class="fa fa-undo"></i></a>
					<?php } ?>
				</td>
	...
	<div class="visible-xs visible-sm">
        ...
        <div class="div-sup-sx-sm">
    		...
    		<select name="o" id="o" class="form-control input-sm select_ordem" onchange="ordem(this.value)">
				<option value="video.id" <?php if($o == "video.id") echo "selected"; ?>>C&oacute;digo</option>
				<option value="video.titulo" <?php if($o == "video.titulo") echo "selected"; ?>>T&iacute;tulo</option>
			</select>
	...
	paginacao($pagina, $totalLinha, $max, $video);
