<?php
    ...
    $capinha = "popup/geral/marca_capinha.php?t=$t";
...
<div class="table-responsive">
	<table class="table table-striped table-hover table-bordered table-condensed">
		<tr class="active">
			<th style="width: 90px;">
				<a href="#" onclick="$('#modal_corpo').load('<?= $capinha ?>&p=<?= $pagina ?>&o=id&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>&time='+$.now()); return false;">
					C&oacute;digo
				</a>
			...
			<th>
				<a href="#" onclick="$('#modal_corpo').load('<?= $capinha ?>&p=<?= $pagina ?>&o=marca&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>&time='+$.now()); return false;">
					Descri&ccedil;&atilde;o
				</a>
				<?php if($o == "marca" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
				<?php if($o == "marca" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
				<form class="form-inline pull-right" action="" method="post" onsubmit="$('#modal_corpo').load('<?= $capinha ?>&busca=' + $('#busca').val() + '&time=' + $.now()); return false;">
...
<?php
    $capinha = $capinha . "&o=$o&d=$d";

    include_once "$root/privado/sistema/classes/includes/paginacao.php";
    paginacao_popup($pagina, $totalLinha, $max, $capinha);
?>
