...
$link = "popup/log/log_listagem.php?t=$t&id=$id";
...
<div class="table-responsive">
	<table class="table table-striped table-hover table-bordered table-condensed">
		<tr class="active">
			...
			<th style="width: 10%;">
				<a href="#" onclick="$('#modal_corpo').load('<?= $link ?>&p=<?= $pagina ?>&o=controle_log_acesso.id_registro&d=<?= empty($d) || $d == "DESC" ? "ASC" : "DESC" ?>&time='+$.now()); return false;">
					Registro
				</a>
				<?php if($o == "controle_log_acesso.id_registro" && $d == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
				<?php if($o == "controle_log_acesso.id_registro" && $d == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
			</th>
			<th style="width: 15%;">
			...
			<th style="width: 15%;" class="text-primary">
				Origem
	...
	try {
		...
		<tr>
			...
			<td class="text-right"><?= $buscaArtigo['id_registro'] ?></td>
			<td><?= $tipoAcesso ?></td>
